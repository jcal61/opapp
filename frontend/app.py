...
from app.services.scheduling import (
    ...
    clock_in, clock_out, get_open_entry, list_time_entries, edit_time_entry,
    labor_cost_summary, scheduled_vs_actual, week_bounds,
)
...
if page == "Dashboard":
    ...
    if require_perm(user, "time_clock_admin"):
        wk_start, wk_end = week_bounds(date.today())
        labor = labor_cost_summary(db, location.id, wk_start, wk_end)
        st.subheader("Labor — this week")
        l1, l2, l3, l4 = st.columns(4)
        l1.metric("Labor Hours", f"{labor['total_hours']:.1f}")
        l2.metric("Labor Cost", f"${labor['total_cost']:,.0f}")
...
# ---------- RECIPES ----------
    ...
    tab_view, tab_manage = st.tabs(["View & Cost", "Manage Recipes"])
...
# ---------- USERS & ROLES ----------
    ...
    with tab1:
        ...
        if not users:
            ...
            if require_perm(user, "time_clock_admin"):
                ...
                st.caption("Used for labor cost / labor % — snapshotted onto each punch at clock-in.")
                if active:
    ...
    groups = {
        ...
        "Scheduling & time clock": [
            "scheduling_view", "scheduling_edit", "time_clock", "time_clock_admin",
        ],
        "Training & quizzes": [
...
# ---------- SCHEDULING ----------
    ...
    with tab_labor:
        wk_start, wk_end = week_bounds(st.session_state.get("sched_week_anchor", date.today()))
        comp = scheduled_vs_actual(db, location.id, wk_start, wk_end)
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Scheduled hrs", f"{comp['scheduled_hours']:.1f}")
        c2.metric("Actual hrs", f"{comp['actual_hours']:.1f}", delta=f"{comp['variance_hours']:+.1f}")
        c3.metric("Scheduled cost", f"${comp['scheduled_cost']:,.0f}")
        c4.metric("Actual cost", f"${comp['actual_cost']:,.0f}", delta=f"${comp['variance_cost']:+,.0f}")
            ...
            st.dataframe(
                pd.DataFrame([
                    {"Employee": r["name"], "Hours": round(r["hours"], 2), "Cost": round(r["cost"], 2),
                     "Currently clocked in": r["open_entry"]}
                    for r in labor["by_user"]
                ]),
                use_container_width=True, hide_index=True,
            )
        if labor["labor_cost_percent"] is not None:
            st.metric("Labor cost % of sales (this week)", f"{labor['labor_cost_percent']:.1f}%")
        else:
            st.caption("No POS sales recorded for this window yet — labor % needs a sales denominator.")

# ---------- TIME CLOCK ----------
elif page == "Time Clock":
    st.title("Time Clock")
    st.caption("Clock in and out, and (for managers) review or correct the team's punches.")

    if not require_perm(user, "time_clock"):
        st.error("You do not have access to the time clock.")
        st.stop()

    tab_me, tab_team = st.tabs(["My clock", "Team punches"])

    with tab_me:
        open_entry = get_open_entry(db, user.id)
        if open_entry:
            elapsed = open_entry.hours()
            st.success(f"Clocked in since **{open_entry.clock_in.strftime('%H:%M')}** — {elapsed:.2f} hrs so far")
            break_min = st.number_input("Break taken (min)", min_value=0, max_value=180, value=open_entry.break_minutes or 0, step=5)
            if st.button("Clock out", type="primary"):
                try:
                    clock_out(db, open_entry.id, break_minutes=int(break_min))
                    st.success("Clocked out")
                    st.rerun()
                except Exception as e:
                    st.error(str(e))
        else:
            st.info("You are not clocked in.")
            if st.button("Clock in", type="primary"):
                try:
                    clock_in(db, user.id, location.id)
                    st.success("Clocked in")
                    st.rerun()
                except Exception as e:
                    st.error(str(e))

        st.subheader("My recent punches")
        wk_start, wk_end = week_bounds(date.today())
        mine = list_time_entries(db, location.id, wk_start, wk_end, user_id=user.id)
        if mine:
            st.dataframe(
                pd.DataFrame([
                    {
                        "In": e.clock_in.strftime("%a %H:%M"),
                        "Out": e.clock_out.strftime("%a %H:%M") if e.clock_out else "— open —",
            ...
            st.caption("No punches yet this week.")

    with tab_team:
        if not require_perm(user, "time_clock_admin"):
            st.warning("Viewing/editing the team's punches requires time_clock_admin permission.")
        else:
            wk_start, wk_end = week_bounds(date.today())
            entries = list_time_entries(db, location.id, wk_start, wk_end)
            if not entries:
                st.info("No punches yet this week.")
            else:
                for e in entries:
                    name = e.user.name if e.user else f"user #{e.user_id}"
                    status = "🟢 open" if e.is_open() else "✅"
                    with st.expander(
                        f"{status} {name} · {e.clock_in.strftime('%a %H:%M')}"
                        f"–{e.clock_out.strftime('%H:%M') if e.clock_out else '…'} · {e.hours():.2f} hrs"
                    ):
                        c1, c2, c3 = st.columns(3)
                        new_in = c1.time_input("Clock in", value=e.clock_in.time(), key=f"in_{e.id}")
                        new_out = c2.time_input(
                            "Clock out", value=(e.clock_out or e.clock_in).time(), key=f"out_{e.id}"
                        )
                        new_break = c3.number_input(
                            "Break (min)", min_value=0, max_value=180, value=e.break_minutes or 0,
                            step=5, key=f"brk_{e.id}",
                        )
                        if st.button("Save correction", key=f"save_{e.id}"):
                            try:
                                edit_time_entry(
                                    db, e.id,
                                    clock_in=datetime.combine(e.clock_in.date(), new_in, tzinfo=timezone.utc),
                                    clock_out=datetime.combine(e.clock_in.date(), new_out, tzinfo=timezone.utc),
                                    break_minutes=int(new_break),
                                )
                                st.success("Updated")
                                st.rerun()
