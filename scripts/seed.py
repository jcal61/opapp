...
from app.services.financials import create_expense, upsert_budget, record_cash_count
from app.services.logbook import create_entry as create_log_entry
...
def seed():
    ...
    db.add(loc)
    db.flush()
    loc2.parent_id = loc.id  # Rooftop Bar rolls up under Main Dining Room for consolidated reporting
    db.flush()
    ...
    # --- Inventory Items ---
    # `department` groups items onto the fixed Stock Snapshot tabs (Food,
    # Prepped Food, Non-Alcoholic Drinks, Alcohol, Paper Goods, Chemicals,
    # Merchandise) — a coarser grouping than the free-text `category` below.
    ...
    items_data = [
        ...
        # Demo coverage for the departments not otherwise represented above.
        ("House Marinara Sauce", "PREP-MARINARA", "prepped", "oz", 0.15, 64),
        ("House Ranch Dressing", "PREP-RANCH", "prepped", "oz", 0.12, 48),
        ("To-Go Container 9in", "PAPER-TOGO9", "paper", "each", 0.22, 200),
        ("Napkins (case)", "PAPER-NAPKIN", "paper", "case", 18.50, 10),
        ("Sanitizer Concentrate", "CHEM-SANITIZER", "chemical", "oz", 0.20, 128),
        ("Dish Soap", "CHEM-DISHSOAP", "chemical", "each", 6.75, 12),
        ("Branded Pint Glass", "MERCH-PINT", "merchandise", "each", 3.50, 48),
        ("Logo T-Shirt (M)", "MERCH-SHIRT-M", "merchandise", "each", 8.00, 24),
    ]
    ...
    for name, sku, cat, unit, cost, par in items_data:
        item = InventoryItem(
            ...
            sku=sku,
            category=cat,
            department=CATEGORY_TO_DEPARTMENT.get(cat, "Food"),
            base_unit=unit,
            current_cost=cost,
            par_level=par,
            preferred_vendor_id=local.id if cat == "produce" else sysco.id,
        )
        db.add(item)
        items[name] = item
    db.flush()
    ...
    from app.services.scheduling import create_shift, week_bounds
    ...
    # --- Financials, cash management & logbook demo data ---
    create_expense(db, loc.id, "Rent & Occupancy", 8500.0, expense_date=today.replace(day=1),
                    description="Monthly base rent", is_recurring=True)
    create_expense(db, loc.id, "Utilities", 1120.0, expense_date=today.replace(day=1),
                    description="Gas & electric", is_recurring=True)
    create_expense(db, loc.id, "Marketing", 400.0, expense_date=today - timedelta(days=3),
                    description="Local social media ads")
    create_expense(db, loc.id, "Repairs & Maintenance", 265.0, expense_date=today - timedelta(days=1),
                    description="Walk-in cooler service call")
    create_expense(db, loc2.id, "Utilities", 380.0, expense_date=today.replace(day=1),
                    description="Rooftop bar — seasonal power")

    upsert_budget(
        db, loc.id, today.year, today.month,
        sales_target=42000.0, food_cost_pct_target=29.0, labor_cost_pct_target=28.0,
        other_expense_target=11000.0, notes="Baseline monthly budget",
    )

    record_cash_count(db, loc.id, today, "Close", expected_amount=612.00, counted_amount=608.50,
                       counted_by_id=mgr_u.id, notes="Slightly short — recounting tomorrow")
    record_cash_count(db, loc.id, today - timedelta(days=1), "Close", expected_amount=584.00, counted_amount=584.00,
                       counted_by_id=server_u.id)

    create_log_entry(db, loc.id, mgr_u.id,
                      "Walk-in compressor was rattling around 3pm. Called the repair vendor — appointment set for tomorrow AM. Keep an eye on temps overnight.",
                      category="Maintenance", pinned=True)
    create_log_entry(db, loc.id, server_u.id,
                      "Table 12 guest had an allergy concern with the Caesar dressing — confirmed no shellfish, guest was satisfied. Noted on their profile.",
                      category="Guest")
    create_log_entry(db, loc.id, kitchen_u.id,
                      "Ran out of romaine mid-dinner rush, 86'd Caesar salad for ~45 min until backup delivery arrived.",
                      category="General", entry_date=today - timedelta(days=1))
    ...
    print("\nFinancials: 5 operating expenses, 1 monthly budget, and 2 cash drawer counts seeded. "
          f"Rooftop Bar ({loc2.code}) is now a child of {loc.name} — see Financials → Consolidated.")
    print("Manager Logbook: 3 demo entries seeded (one pinned maintenance note).")
    print(f"Inventory: {len(items)} items seeded across all 7 departments (Food, Prepped Food, "
          "Non-Alcoholic Drinks, Alcohol, Paper Goods, Chemicals, Merchandise) — see Inventory & Stock.")
