...
from app.models import (
    InventoryItem, StockLevel, Location, DEPARTMENTS,
...
def create_item(
    ...
    if department and department.strip() and department.strip() not in DEPARTMENTS:
        raise ValueError(f"'{department}' isn't a recognized department. Choose one of: {', '.join(DEPARTMENTS)}.")
    item = InventoryItem(
        name=name.strip(),
...
def update_item(
    ...
    if sku and sku.strip():
        ...
        if dupe:
            raise ValueError(f"SKU '{sku}' is already used by another item.")
    if department is not ... and department and department.strip() and department.strip() not in DEPARTMENTS:
        raise ValueError(f"'{department}' isn't a recognized department. Choose one of: {', '.join(DEPARTMENTS)}.")
    if name is not None:
...
# Keyword hints used to guess a department from an item's existing free-text
# category/subcategory/name — lets existing data (or a bulk import that only
# ever set `category`) get sorted into the new tabs without hand-editing
# every item. Checked in order; first match wins.
_DEPARTMENT_HINTS = [
    ("Alcohol", ["liquor", "beer", "wine", "spirit", "vodka", "whiskey", "whisky", "bourbon", "tequila", "rum", "gin", "vermouth", "liqueur"]),
    ("Non-Alcoholic Drinks", ["beverage", "soda", "juice", "coffee", "tea", "water", "drink mixer", "syrup soda"]),
    ("Paper Goods", ["paper", "to-go", "togo", "napkin", "container", "cup", "straw", "utensil", "packaging"]),
    ("Chemicals", ["chemical", "clean", "sanitiz", "detergent", "degreaser", "soap"]),
    ("Merchandise", ["merch", "apparel", "shirt", "glassware branded", "retail"]),
    ("Prepped Food", ["prepped", "prep ", "house-made", "house made", "batch", "sauce", "dressing", "marinade", "stock", "brine"]),
]


def suggest_department(name: str = "", category: str = "", subcategory: str = "") -> str:
    """Best-guess department from existing free-text fields — used as a
    default suggestion, never applied silently without going through
    auto_assign_departments or an explicit save."""
    haystack = f"{category or ''} {subcategory or ''} {name or ''}".lower()
    for dept, keywords in _DEPARTMENT_HINTS:
        if any(kw in haystack for kw in keywords):
            return dept
    return "Food"  # the safest default — most inventory items are raw food


def auto_assign_departments(db: Session) -> int:
    """Bulk-assign a department to every item that doesn't have one
    yet, using suggest_department(). Returns how many were updated. Doesn't
    touch items that already have a department — re-run after fixing any
    mis-guesses and it will leave those alone."""
    q = db.query(InventoryItem).filter(InventoryItem.department.is_(None))
    updated = 0
    for item in q.all():
        item.department = suggest_department(item.name, item.category, item.subcategory)
        updated += 1
    if updated:
        db.commit()
    return updated
