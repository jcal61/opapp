...
class ChecklistTaskTemplate(Base):
    ...
    # Tasks with completion history can't be safely hard-deleted (their
    # ChecklistTaskCompletion rows would be orphaned), so they get archived
    # via this flag instead — hidden from new/open runs but still visible on
    # completed runs where they were actually done.
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
