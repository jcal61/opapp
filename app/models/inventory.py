...
# Top-level inventory departments — coarser than `category` (which is
# free-text, e.g. "produce", "liquor", "dry") and used to organize the
# Inventory & Stock page into separate browsable tabs. Kept as a fixed list
# (rather than free text like category/subcategory) so the tabs are stable
# and predictable regardless of how items get categorized.
DEPARTMENTS = [
    "Food",
    "Prepped Food",
...
class InventoryItem(Base):
    ...
    subcategory: Mapped[str | None] = mapped_column(String(80))  # e.g. Vodka, Spices, Keg
    department: Mapped[str | None] = mapped_column(String(40))  # one of DEPARTMENTS, or None = Uncategorized
