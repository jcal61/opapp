...
class Invoice(Base):
    ...
    # Raw JSON returned by the AI extraction step, kept for audit/debugging
    # so a manager can see exactly what the model read off the document.
    ai_extraction_raw: Mapped[str | None] = mapped_column(Text)
...
class InvoiceLine(Base):
    ...
    # "quantity" is the shipped/billed quantity (what you're actually charged
    # for and what feeds line_total + PO matching). quantity_ordered is the
    # separate "ordered" figure some vendor invoices print alongside it,
    # useful for spotting backorders/shortages before running a PO match.
    quantity: Mapped[float] = mapped_column(Float, nullable=False)
    quantity_ordered: Mapped[float | None] = mapped_column(Float)
